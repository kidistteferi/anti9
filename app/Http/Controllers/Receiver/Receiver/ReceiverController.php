...
namespace App\Http\Controllers\Receiver\Receiver;
    ...
    public function commentrec(Request $request){
        //    // $result = Message::where('id',$id)->get();comment
            ...
            $comment = $request['comment_rec'];
              $result = Head::where('mid',$mid)->update([
                 'comment_rec' => $comment,
               'accept' => '1'
               ]);
            //    dd($result);
...
// public function ant($mid){
//     $mid = $request['mid'];
//     $comment = $request['comment_rec'];
//     $result2 = Head::find($mid);
//     return redirect('receiver.show_fields')->with('result2',$result2)->with('mid',$mid);
...
// }
    ...
    {
        $receivers = Complaints::join('messages','messages.c_id','complaints.id')
        ->get();
        
        return view('receiver.receiver.receivers.index')
    ...
    {
        $receiver = Message::where('id',$id)->get();
        ...
        return view('receiver.receiver.receivers.show')->with('receiver', $receiver);
    }
